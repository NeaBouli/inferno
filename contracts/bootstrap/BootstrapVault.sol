...
///
///         IMPORTANT: ifrSource must approve this contract for 2 * ifrAllocation IFR.
///         Half goes to LP creation (paired with all ETH), half to contributor claims.
contract BootstrapVault is ReentrancyGuard {
    IERC20 public immutable ifrToken;
    address public immutable ifrSource;
    ...
    /// @notice Deploy BootstrapVault with immutable parameters
    /// @param _ifrToken IFR token address
    /// @param _ifrSource Address holding IFR (must approve 2 * _ifrAllocation)
    ...
    constructor(
        ...
        require(_ifrSource != address(0), "ifrSource=0");
    ...
    /// @notice Finalise the bootstrap: pull IFR, create LP, lock LP tokens.
    ...
    function finalise() external nonReentrant {
        ...
        // Pull IFR: ifrAllocation for LP + ifrAllocation for claims
        require(
            ifrToken.transferFrom(ifrSource, address(this), ifrAllocation * 2),
            "IFR pull failed"
        );
