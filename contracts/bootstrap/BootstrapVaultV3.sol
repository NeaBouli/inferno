...
///
///         V3 change: Added permissionless refund() after 30-day grace period.
///         If finalise() is never called within 30 days after endTime,
///         contributors can reclaim their ETH without admin intervention.
contract BootstrapVaultV3 is ReentrancyGuard {
    ...
    /// @notice Deploy BootstrapVaultV3 with immutable parameters
    ...
    /// @notice Refund ETH to a contributor if finalise() was never called.
    /// @dev Trustless grace period escape hatch. If finalise() was never
    ///      called 30 days after endTime, contributors can self-rescue their ETH.
    ///      Permissionless — no admin required.
    function refund() external nonReentrant {
        require(!finalised, "already finalised");
        require(block.timestamp > endTime + REFUND_GRACE_PERIOD, "grace period active");
        require(contributions[msg.sender] > 0, "no contribution");
