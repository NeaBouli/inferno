...
- Benefits integration: the live customer/seller PWA is https://shop.ifrunit.tech/. Seller-issued QR codes contain a short-lived local customer proof URL; the customer signs, the backend checks the selected IFRLock threshold, and the seller redeems an approval once. Cross-device customer history uses a separate single-use signature exchange and signer-bound snapshot pages; its bearer access remains memory-only for ten minutes. Seller rewards remain governance-gated.
