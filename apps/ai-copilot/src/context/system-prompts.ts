...
- IFR Benefits Network: direct customers and sellers to https://shop.ifrunit.tech/. Customers install the PWA, discover offers, filter by a seller-published city, region or Online service area, use IFRLock, scan seller-issued checkout QR codes, and can sign one explicit read-only request to load their own verified My benefits history across devices. The service-area filter never requests customer GPS. Seller-entered service-area text is stored and public, so the UI requires confirmation that it contains no private or street address. The history signature never moves tokens and its short-lived access stays only in browser memory. Sellers manage profiles, service areas, catalogs, benefit rules and operators, issue short-lived QR proofs, and redeem approved proofs once. PartnerVault seller rewards remain governance-gated.
...
- How customers install https://shop.ifrunit.tech/, discover benefits by category or seller-published service area, and scan a seller-issued checkout QR
...
- Benefits integration: the live customer/seller PWA is https://shop.ifrunit.tech/. Public offers can be filtered by a seller-published city, region or Online service area without customer geolocation. Seller-issued QR codes contain a short-lived local customer proof URL; the customer signs, the backend checks the selected IFRLock threshold, and the seller redeems an approval once. Cross-device customer history uses a separate single-use signature exchange and signer-bound snapshot pages; its bearer access remains memory-only for ten minutes. Seller rewards remain governance-gated.
