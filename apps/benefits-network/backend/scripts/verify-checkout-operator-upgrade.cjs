...
const targetMigration = '20260717010700_add_checkout_operators';
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'benefits-operator-upgrade-'));
...
try {
  ...
  sqlite(`
    ...
    INSERT INTO Session (
      id, businessId, nonce, expiresAt, status, createdAt, updatedAt, attestAttempts
    ) VALUES (
      'existing-session', 'migration-fixture', 'existing-nonce',
  ...
  `);
  if (preservedRows !== '1|1|1') {
    throw new Error(`Existing rows were not preserved: ${preservedRows}`);
  }
  if (sqlite("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='CheckoutOperator';") !== '1') {
    throw new Error('CheckoutOperator table was not created');
  }
  const foreignKeyErrors = sqlite('PRAGMA foreign_key_check;');
  if (foreignKeyErrors) {
    throw new Error(`Foreign-key errors after migration:\n${foreignKeyErrors}`);
  }

  console.log('Checkout operator migration upgrade fixture passed');
} finally {
