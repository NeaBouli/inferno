...
async function verifyHttpSurface() {
  ...
  assert((await serviceWorker.text()).includes("ifr-benefits-v21"), 'service worker cache version mismatch');
  log('PWA assets OK');
...
async function verifyPage(contextOptions, label) {
  ...
  try {
    ...
    await expectText(page, 'Example tier guide');
    ...
    await expectText(page, 'Example lock amounts');
    await expectText(page, 'Silver / 2,500 IFR');
    await expectText(page, 'Platinum / 10,000 IFR');
    await expectText(page, 'Sellers define each real rule and may use different thresholds.');
    ...
    await codeGenerator.getByLabel('Business ID').fill('seller-smoke');
    await codeGenerator.getByLabel('Benefit rule ID').fill('rule-smoke');
    await codeGenerator.getByRole('button', { name: 'Scanner', exact: true }).click();
    await expectText(page, 'https://shop.ifrunit.tech/b/seller-smoke');
    await codeGenerator.getByRole('button', { name: 'REST', exact: true }).click();
    await expectText(page, 'Request a one-time, rule-bound challenge');
    await expectText(page, 'Required headers');
    await expectText(page, 'x-ifr-nonce');
    await codeGenerator.getByRole('button', { name: 'SDK / POS', exact: true }).click();
    ...
    await expectText(page, 'Open an existing seller setup');
    await expectText(page, 'Connect before entering a new seller profile');
    await expectText(page, 'Finish the seller profile first');
    await page.locator('details').filter({ hasText: 'Open an existing seller setup' }).locator('summary').click();
    await page.getByPlaceholder('Business ID, seller URL or backup JSON').fill('smoke-manual-business');
    await page.getByRole('button', { name: 'Restore seller reference', exact: true }).click();
    await expectText(page, 'Load existing seller profile');
    await expectText(page, 'Load existing profile');
    await page.locator('details').filter({ hasText: 'Public discovery details' }).locator('summary').click();
